...
transactions[index][1][:days_offset] = rand(2..4)
...
responses_chargeback.map(&:first).all?('200')

transactions_refused = responses_transactions.filter { |rt|
  JSON.parse(rt[1])['recommendation'] == 'refuse'
}.count.to_f / responses_transactions.count
responses_transactions.filter do |rt|
  JSON.parse(rt[1])['recommendation'] == 'flagged'
end.count.to_f
responses_transactions.count
